...
assignment4 = Blueprint('assignment4',
                         __name__,
...
def assignment4_def():
    ...
    if 'name' in request.args:
        ...
        return render_template('assignment4.html', users=users, name=name)
    elif 'email' in request.args:
        email = request.args['email']
    return render_template('assignment4.html', users=users, email=email)
...
def insert_user_func():
    ...
    return redirect('/assignment4?name='+name)


@assignment4.route('/delete_users', methods=['POST'])
def delete_users_func():
    email = request.form['email']
    qurey = " DELETE FROM users WHERE email='%s';" % email
    interact_db(query=qurey, query_type='commit')

    return redirect('/assignment4?email='+email)
